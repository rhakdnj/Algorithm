    ...
    while dq:
        ...
        for i in range(4):
            ...
            if ny < 0 or nx < 0 or ny >= R or nx >= C:
                continue
            if fire_visited[ny][nx] != INF or arr[ny][nx] == '#':
                continue
            fire_visited[ny][nx] = fire_visited[y][x] + 1
            dq.append((ny, nx))

    ret = 0
    man_visited = [[INF] * C for _ in range(R)]
    man_visited[sy][sx] = 1
    dq.append((sy, sx))
    while dq:
        y, x = dq.popleft()
        ...
        for i in range(4):
            ...
            if ny < 0 or nx < 0 or ny >= R or nx >= C:
                continue
            if man_visited[ny][nx] != INF or arr[ny][nx] == '#':
                continue
            if man_visited[y][x] + 1 >= fire_visited[ny][nx]:
                continue
